      ...
      <div className="flex items-start justify-between mb-4">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <span className="text-sm font-semibold text-[white]">{props.locationName}</span>
          </div>
          {props.summary && (
            <div className="text-xs text-[rgba(255,255,255,0.7)] mb-1" title={props.summary}>
              {props.summary}
            </div>
          )}
  ...
  return (
    <div className="space-y-1">
      <div className="flex items-center justify-between gap-1">
        <div className="text-xs font-semibold text-[white] truncate" title={props.locationName}>
          {props.locationName}
        </div>
        <div className="flex items-center gap-1">{props.actions}</div>
      </div>
      {props.summary && (
        <div className="text-xs text-[rgba(255,255,255,0.5)] truncate" title={props.summary}>
          {props.summary}
        </div>
      )}
    </div>
