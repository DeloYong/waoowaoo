  ...
  return (
    <div className="p-4 space-y-2">
      <div className="flex items-center justify-between text-xs">
        <span className="px-2 py-0.5 bg-[var(--wuhu-neon-purple)]/20 text-[var(--wuhu-neon-purple)] rounded font-medium">{panel.textPanel?.shot_type || t('panelCard.unknownShotType')}</span>
        {panel.textPanel?.duration && <span className="text-white/50">{panel.textPanel.duration}{t('promptModal.duration')}</span>}
      </div>

      <p className="text-sm text-white/70 line-clamp-2">{panel.textPanel?.description}</p>

      <div className="mt-3 pt-3 border-t border-[var(--wuhu-neon-purple)]/20">
        {(showsIncomingLinkBadge || showsOutgoingLinkBadge) && (
          <div className="mb-2 flex flex-wrap gap-1.5">
            {showsIncomingLinkBadge && (
              <span
                className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-medium ${showsOutgoingLinkBadge
                    ? 'bg-[var(--wuhu-neon-purple)]/20 text-[var(--wuhu-neon-purple)]'
                    : 'bg-[var(--wuhu-bg-surface)] text-white/50 border border-white/20'
                  }`}
              >
            ...
            {showsOutgoingLinkBadge && (
              <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-medium bg-[var(--wuhu-neon-purple)]/20 text-[var(--wuhu-neon-purple)]">
        ...
        {showsPromptEditor && (
          <>
            <div className="flex items-center justify-between mb-1">
              <span className="text-xs font-medium text-white/50">{t('promptModal.promptLabel')}</span>
              {!promptEditor.isEditing && (
                <button onClick={promptEditor.handleStartEdit} className="text-white/50 hover:text-[var(--wuhu-neon-purple)] transition-colors p-0.5">
            ...
            {promptEditor.isEditing ? (
              <div className="relative mb-3">
                ...
                <div className="absolute right-1 top-1 flex flex-col gap-1">
                  <button onClick={promptEditor.handleSave} disabled={promptEditor.isSavingPrompt} className="px-2 py-1 text-[10px] bg-gradient-to-r from-[var(--wuhu-neon-purple)] to-[var(--wuhu-neon-pink)] text-white rounded">{promptEditor.isSavingPrompt ? '...' : t('panelCard.save')}</button>
                  <button onClick={promptEditor.handleCancelEdit} disabled={promptEditor.isSavingPrompt} className="px-2 py-1 text-[10px] bg-[var(--wuhu-bg-surface)] text-white/70 rounded">{t('panelCard.cancel')}</button>
                </div>
              </div>
            ) : (
              <div onClick={promptEditor.handleStartEdit} className="text-xs p-2 border border-white/20 rounded-lg bg-[var(--wuhu-bg-surface)] text-white/70 cursor-pointer">
                {promptEditor.localPrompt || <span className="text-white/50 italic">{t('panelCard.clickToEditPrompt')}</span>}
              </div>
            )}
              ...
              return (
                <div className="mt-2 flex items-center gap-2">
                  <button
                    ...
                    className="flex-shrink-0 min-w-[120px] py-2 px-3 text-sm font-medium rounded-lg shadow-sm transition-all disabled:opacity-50 bg-gradient-to-r from-[var(--wuhu-neon-purple)] to-[var(--wuhu-neon-pink)] text-white"
                  >
              ...
              <>
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="flex-shrink-0 min-w-[90px] py-2 px-3 text-sm font-medium rounded-lg shadow-sm transition-all disabled:opacity-50 bg-gradient-to-r from-[var(--wuhu-neon-purple)] to-[var(--wuhu-neon-pink)] text-white"
                  >
                  ...
                  <div className="mt-2">
                    <div className="flex gap-2">
                      <button
                        ...
                        className="flex-1 py-1.5 text-xs rounded-lg transition-all flex items-center justify-center gap-1 bg-gradient-to-r from-[var(--wuhu-neon-purple)] to-[var(--wuhu-neon-pink)] text-white disabled:opacity-50"
                      >
                    ...
                    {voiceManager.localVoiceLines.length > 0 && (
                      <div className="mt-2 space-y-1">
                        {voiceManager.localVoiceLines.map((voiceLine) => {
                          ...
                          return (
                            <div key={voiceLine.id} className="flex items-start gap-1.5 p-1.5 bg-[var(--wuhu-bg-surface)] rounded text-[10px]">
                              {voiceLine.audioUrl ? (
                                <button
                                  onClick={(event) => {
                                  ...
                                  className="flex-shrink-0 w-5 h-5 rounded-full flex items-center justify-center transition-colors bg-[var(--wuhu-bg-surface)]"
                                ...
                                <button
                                  ...
                                  className="flex-shrink-0 px-1.5 py-0.5 bg-gradient-to-r from-[var(--wuhu-neon-purple)] to-[var(--wuhu-neon-pink)] text-white rounded disabled:opacity-50"
                              ...
                              <div className="flex-1 min-w-0">
                                <span className="text-white/50">{voiceLine.speaker}: </span>
                                <span className="text-white/70">&ldquo;{voiceLine.content}&rdquo;</span>
                              </div>
                            </div>
                          )
                        })}
