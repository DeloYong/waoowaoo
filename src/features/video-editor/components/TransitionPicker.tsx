                ...
                {TRANSITION_OPTIONS.map(option => (
                    <button
                        ...
                        style={{
                            ...
                            gap: '2px',
                            padding: '8px 4px',
                            background: value === option.type ? 'var(--wuhu-neon-purple)' : 'rgba(255,255,255,0.05)',
                            border: value === option.type ? '1px solid var(--wuhu-neon-cyan)' : '1px solid rgba(167, 87, 255, 0.2)',
                            borderRadius: '6px',
                    ...
                    >
                        <AppIcon
                            name={option.icon}
                            size={16}
                            color={value === option.type ? 'white' : 'white'}
                        />
                        <span style={{ fontSize: '10px', color: value === option.type ? 'white' : 'white' }}>{t(`editor.transition.options.${option.labelKey}`)}</span>
                    </button>
                ))}
            </div>
                        ...
                        {DURATION_OPTIONS.map(option => (
                            <button
                                ...
                                style={{
                                    flex: 1,
                                    padding: '6px 8px',
                                    background: duration === option.value ? 'var(--wuhu-neon-purple)' : 'rgba(255,255,255,0.05)',
                                    border: duration === option.value ? '1px solid var(--wuhu-neon-cyan)' : '1px solid rgba(167, 87, 255, 0.2)',
                                    borderRadius: '4px',
                                    fontSize: '11px',
                                    color: duration === option.value ? 'white' : 'white',
                                    cursor: disabled ? 'not-allowed' : 'pointer',
                                    opacity: disabled ? 0.5 : 1
                                }}
                            >
